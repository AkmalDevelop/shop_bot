...
@dp.pre_checkout_query(lambda quary: True)
...
async def get_payment(message: Message):
    ...
    if drop_cart_products_default(cart_id):
        await bot.send_message(chat_id, text='Оплата прошла успешно:)')
...
async def create_order(call: CallbackQuery):
    ...
    data_order = datetime.datetime.now().strftime('%d.%m.%Y')

    cart_products = get_cart_products(cart_id)
    total_products, total_price = get_total_products_price(cart_id)

    save_order_check(cart_id, total_price, total_products, time_order, data_order)
...
Общая стоимость: {final_price}\n\n'''
    ...
    text += f'''Общее количество заказанных продуктов: {0 if total_products is None else total_products}
Итог: {0 if total_price is None else total_price}'''
